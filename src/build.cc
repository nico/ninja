...
bool Builder::Build(string* err) {
  ...
  int num_failures_allowed = config_.num_failures_allowed;
  ...
  while (plan_.more_to_do()) {
    ...
    if (num_failures_allowed > 0 && command_runner_->CanRunMore()) {
    ...
    if (num_failures_allowed == 0) {
      if (config_.num_failures_allowed > 1)
        *err = "subcommands failed";
      else
        *err = "subcommand failed";
    } else if (num_failures_allowed < config_.num_failures_allowed)
      *err = "cannot make progress due to previous errors";
    else
      *err = "stuck [this is a bug in ninja itself]";
